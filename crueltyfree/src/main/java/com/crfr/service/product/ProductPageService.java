...
@Service("pdPage")
public class ProductPageService implements ProductService {
	
	public PageNav setPageNav(PageNav pageNav, String pageNum, String pageBlock, ExploreVo vo) {
		int totalRows = pageNav.getTotalRows();
		int rows_per_page = 15;
			if(vo.getSort_view()==30) {
				rows_per_page = 30;
			}else if(vo.getSort_view()==45) {
				rows_per_page = 45;
			}else if(vo.getSort_view()==15) {
				rows_per_page = 15;
			}
