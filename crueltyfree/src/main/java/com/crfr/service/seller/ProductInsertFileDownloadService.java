...
@Service("pFileDownload")
public class ProductInsertFileDownloadService implements ProductSellerService {
	
	public void download (String originFile, String saveFile, 
		...
		try{
			...
			if(client.indexOf("WOW64")== -1){//WOW64: Windows on Windows 64-bit, 64비트 버전의 Windows에서 
				...
				originFile = new String(originFile.getBytes("UTF-8"), "ISO-8859-1");
				//HTTP의 정보는 ASCII로 인코딩되므로 여기에 문자셋을 맞추기 위해서 먼저 파일 이름을 UTF-8로 된 배열 생성후
				//ISO-8859-1문자셋으로 문자열 객체를 생성함
			}else{
				originFile = new String(originFile.getBytes("KSC5601"), "ISO-8859-1");
			}
			
			//파일 다운로드용 응답헤더 설정
			response.reset();
			response.setContentType("application/octet-stream");//파일 다운로드를 위한 MIME타입 설정
			response.setHeader("Content-Disposition", "attachment; filename=\""+originFile+"\"");
