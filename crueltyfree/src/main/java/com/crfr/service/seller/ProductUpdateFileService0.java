...
@Service("pUpdateProductFile0")
@RequiredArgsConstructor
public class ProductUpdateFileService0 implements ProductSellerService {
	private final ProductSellerDao dao;
	
	public int productUpdateFile0(MultipartFile attachedFile0,
			String file_idx, HttpServletRequest request) {
		
			int result = 0;
			if(!attachedFile0.isEmpty()) {
				//1. 저장 디렉터리에 저장할 새로운 파일명 만들기
				String originFileName = attachedFile0.getOriginalFilename(); //원본 파일 이름
				...
				if(originFileName.length() != 0) {//새로운 파일을 선택한 경우
					...
					try {
						attachedFile0.transferTo(new File(fullPath));//지정된 경로에 파일 저장
				...
				result = dao.productUpdateFile0(vo);
			}
