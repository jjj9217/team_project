...
<body>
	...
	<div id="contentcontainer">
		<fieldset>
            ...
            <span id="sub-title">작성 글 보기</span>&nbsp;&nbsp;
            <span id="link2"><a href="../membership/myreply.do">작성 댓글 보기</a>&nbsp;&nbsp;</span>	            
	            <form name="adminfrm">
	            <table id="tb_board">
	                <tr>
	                	 <c:set var="total" value="${totalRows}" />
	                    <td id="td_count" colspan="4">Total: ${total}</td>
	                </tr>
	                <tr id="tr_blue_color">
				...
				<tr class="tr_sky_color">
					<td class="td_number">${contentList[rowNum-1].all_board_number}</td>
					<td class="td_title">
						<c:choose>
							<c:when test="${contentList[rowNum-1].board_initial eq 'F'}">
							<a href="../free_board/view.do?no=${contentList[rowNum-1].board_idx}">								
							${contentList[rowNum-1].title}
							<c:if test="${!empty contentList[rowNum-1].originFile}">
								<img src="../resources/img/attachedfile.png" width="12px" height="12px">
							</c:if>
							<c:choose>
								<c:when test="${contentList[rowNum-1].reply_count eq 0}">
								</c:when>
								<c:otherwise>
									&nbsp;&nbsp;<span id="reply_color">${contentList[rowNum-1].reply_count}</span>
								</c:otherwise>
							</c:choose>
							</a>
							</c:when>
							<c:when test="${contentList[rowNum-1].board_initial eq 'Q'}">
							<a href="../question_board/view.do?no=${contentList[rowNum-1].board_idx}">								
							${contentList[rowNum-1].title}
							<c:if test="${!empty contentList[rowNum-1].originFile}">
								<img src="../resources/img/attachedfile.png" width="12px" height="12px">
							</c:if>
							...
							<c:when test="${contentList[rowNum-1].board_initial eq 'R'}">
							<a href="../review_board/view.do?no=${contentList[rowNum-1].board_idx}">								
							${contentList[rowNum-1].title}
							<c:if test="${!empty contentList[rowNum-1].originFile}">
								<img src="../resources/img/attachedfile.png" width="12px" height="12px">
							</c:if>
							...
							<c:when test="${contentList[rowNum-1].board_initial eq 'O'}">
							<a href="../oneline_board/view.do?no=${contentList[rowNum-1].board_idx}">								
							${contentList[rowNum-1].title}
							<c:if test="${!empty contentList[rowNum-1].originFile}">
								<img src="../resources/img/attachedfile.png" width="12px" height="12px">
							</c:if>
					...
					<td class="td_read_count">${contentList[rowNum-1].read_count}</td>
					<td class="td_date">${contentList[rowNum-1].post_date}</td>			
				</tr>				
				</c:forEach>
				<tr><td colspan="4" class="td_space"></tr>				
	            </table>
	            </form>
	            <table id="tb_search">
					<tr>
						<td id="td_paging" colspan="4">
							<%@ include file="../membership/paging_mycontent.jsp" %>
						</td>
					</tr>
					<tr>
