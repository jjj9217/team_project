...
<%@ page import="vo.QuestionBoardVo, dao.QuestionBoardDao" %>
...
$(function(){
	...
	$("#postQuantity").change(function(){
    	...
    	location.href="list"+ rows_per_page + ".do" ;
	});	
			...
			<c:choose>
				...
				<c:otherwise>
					...
					<c:forEach var="rowNum" begin="${startNum}" end="${endNum}">
						<tr class="rowColor">
							<td class="td_num">${boardList[rowNum-1].board_idx}</td>
							<td class="td_title">
							...
							<td class="td_nickname">${boardList[rowNum-1].member_nickname}</td>
							<td class="td_count">${boardList[rowNum-1].recommendation_count}</td>
							<td class="td_count">${boardList[rowNum-1].read_count}</td>
							<fmt:formatDate value="${boardList[rowNum-1].post_date}" type="date" pattern="yyyy-MM-dd HH:mm" var="formattedDate"/>
							<td class="td_date">${formattedDate}</td>					
						</tr>				
					</c:forEach>				
				</c:otherwise>
			</c:choose>							
			...
			<tr>
				...
				<td id="td_search" colspan="4">
				 	<c:choose>							
						<c:when test="${!empty membership}">
						<!-- 회원용 -->
				 			<a href="../question_board/write.do"><input type="button" name="write_btn" id="write_btn" value="글쓰기"></a>
						</c:when>
						<c:otherwise>	
						<!-- 일반용 -->
						</c:otherwise>
	...
	<div id="pagingcontainer">	
		<div id="pagingleft" class="pull-left">
			<table>
				<tr>
					<td id="td_paging">
						<%@ include file="../question_board/paging_list30.jsp" %>					
					</td>
				</tr>
			</table>					
		</div>
