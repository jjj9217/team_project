...
<head>
<meta charset="UTF-8">
<title>SkyBox - "${questionBoardVo.title}" - 질문게시판</title>
...
function deletePost() {
    ...
    if (confirmAns) {
        ...
        frm_view.action = "delete_process.do?no=${questionBoardVo.board_idx}";
        frm_view.submit(); 
...
<body>
	...
	<div id="contentcontainer">
		...
		<table id="tb_board">			
			<tr>
				<td id="td_title" colspan="2"><a href="../question_board/list.do">질문게시판</a></td>
				<c:choose>							
					<c:when test="${(membership.grade eq 1) or (!empty membership and (membership.member_idx eq questionBoardVo.member_idx))}">
						<td id="td_title_side" colspan="3">
							<a href="../question_board/edit.do?no=${questionBoardVo.board_idx}"><input type="button" name="edit" id="edit" value="글수정"></a>
							<input type="button" onclick="deletePost();" name="delete" id="delete" value="글삭제">
							<a href="../question_board/list.do"><input type="button" name="list" id="list" value="목록"></a>
						</td>
					</c:when>
					<c:otherwise>
						<td id="td_title_side" colspan="3">
			...
			<tr id="tr_title">
				<td id="td_title_text" colspan="2">
					${questionBoardVo.title}
				</td>
				<td id="td_title_text_side" colspan="3">
					${questionBoardVo.member_nickname}
				</td>
			</tr>			
			<tr>
				<td id="td_content_info" colspan="2"></td>
				<td id="td_count">
				추천수 : <span id="rd_count">${questionBoardVo.recommendation_count}</span>	
				</td>
				<td id="td_count">
				조회수 : ${questionBoardVo.read_count}
				</td>
				<fmt:formatDate value="${questionBoardVo.post_date}" type="date" pattern="yyyy-MM-dd HH:mm" var="formattedDate"/>
				<td id="td_date">${formattedDate}</td>
			</tr>				
			<tr>
				<td id="td_content" colspan="5">					
					<c:if test="${!empty questionBoardVo.saveFile}">
						<img src="../resources/uploads/${questionBoardVo.saveFile}" width="200px" height="200px"  alt="이미지가 없는 첨부파일"><br>
					</c:if>
					<br>
					${questionBoardVo.content}
				</td>
			</tr>
			</table>
			</form>
			...
			<tr>
				<td class="td_recommendation" colspan="2">
					<form name="rd_up_frm" method="post" action="rd_up_process.do?no=${questionBoardVo.board_idx}">
					<input type="hidden" name="member_idx" id="member_idx" value="${membership.member_idx}">
					<input type="hidden" name="board_idx" id="rd_up_board_idx" value="${questionBoardVo.board_idx}">
					<input type="button" class="rd_up_button" name="rd_up" id="rd_up" value="추천">
					</form>
					<input type="button" class="rd_up_button"  name="rd_up_count" id="rd_up_count" value="${questionBoardVo.recommendation_count}" disabled>
				</td>
				<td class="td_center"></td>  				
				<td class="td_not_recommendation" colspan="2">				
					<form name="rd_up_frm" method="post" action="rd_down_process.do?no=${questionBoardVo.board_idx}">
					<input type="hidden" name="member_idx" id="member_idx" value="${membership.member_idx}">
					<input type="hidden" name="board_idx" id="rd_down_board_idx" value="${questionBoardVo.board_idx}">
					<input type="button" class="rd_down_button" name="rd_down" id="rd_down" value="비추천">
					</form>
					<input type="button" class="rd_down_button" name="rd_down_count" id="rd_down_count" value="${questionBoardVo.not_recommendation_count}" disabled>		
				</td>
			</tr>
			</c:when>
			<c:otherwise>
			...
			<tr>
				<td id="td_file" colspan="5">					
					첨부파일:
					<c:choose>
						<c:when test="${!empty questionBoardVo.originFile}">
							<a href="download.do?oName=${questionBoardVo.originFile}&sName=${questionBoardVo.saveFile}">
							${questionBoardVo.originFile}</a>
						</c:when>
						<c:otherwise>
						없음
						</c:otherwise>
				...
				<tr>
					<td class="reply_content" colspan="5">
						<form class="reply_edit_frm_class" name="reply_edit_frm">
							...
							<input type="hidden" name="board_idx" id="board_idx" value="${questionBoardVo.board_idx}">
			...
			<tr>
				<td id="td_reply_paging" colspan="5">
					<%@ include file="../question_board/paging_reply.jsp" %>	
				</td>
			</tr>
			</table>
