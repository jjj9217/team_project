...
<head>
<meta charset="UTF-8">
<title>SkyBox - "${reviewBoardVo.title}" - 리뷰</title>
...
function deletePost() {
    ...
    if (confirmAns) {
        ...
        frm_view.action = "delete_process.do?no=${reviewBoardVo.board_idx}";
        frm_view.submit(); 
...
<body>
	...
	<div id="contentcontainer">
		...
		<table id="tb_board">			
			<tr>
				<td id="td_title" colspan="2"><a href="../review_board/list.do">리뷰</a></td>
				<c:choose>							
					<c:when test="${(membership.grade eq 1) or (!empty membership and (membership.member_idx eq reviewBoardVo.member_idx))}">
						<td id="td_title_side" colspan="3">
							<a href="../review_board/edit.do?no=${reviewBoardVo.board_idx}"><input type="button" name="edit" id="edit" value="글수정"></a>
							<input type="button" onclick="deletePost();" name="delete" id="delete" value="글삭제">
							<a href="../review_board/list.do"><input type="button" name="list" id="list" value="목록"></a>
						</td>
					</c:when>
					<c:otherwise>
						<td id="td_title_side" colspan="3">
			...
			<tr id="tr_title">
				...
				<td id="td_title_text_side" colspan="3">
					${reviewBoardVo.member_nickname}
				</td>
			</tr>			
			<tr>
				<td id="td_content_info" colspan="2"></td>
				<td id="td_count">
				추천수 : <span id="rd_count">${reviewBoardVo.recommendation_count}</span>	
				</td>
				<td id="td_count">
				조회수 : ${reviewBoardVo.read_count}
				</td>
				<fmt:formatDate value="${reviewBoardVo.post_date}" type="date" pattern="yy-MM-dd HH:mm" var="formattedDate"/>
				<td id="td_date">${formattedDate}</td>
			</tr>				
			<tr>
				<td id="td_content" colspan="5">					
					<c:if test="${!empty reviewBoardVo.saveFile}">
						<img src="../resources/uploads/${reviewBoardVo.saveFile}" width="200px" height="200px"  alt="이미지가 없는 첨부파일"><br>
					</c:if>
					<br>
			...
			<tr>
				<td class="td_recommendation" colspan="2">
					<form name="rd_up_frm" method="post" action="rd_up_process.do?no=${reviewBoardVo.board_idx}">
					<input type="hidden" name="member_idx" id="member_idx" value="${membership.member_idx}">
					<input type="hidden" name="board_idx" id="rd_up_board_idx" value="${reviewBoardVo.board_idx}">
					<input type="button" class="rd_up_button" name="rd_up" id="rd_up" value="추천">
					</form>
					<input type="button" name="rd_up_count" id="rd_up_count" value="${reviewBoardVo.recommendation_count}" disabled>
				</td>
				<td class="td_center"></td>  				
				<td class="td_not_recommendation" colspan="2">				
					<form name="rd_up_frm" method="post" action="rd_down_process.do?no=${reviewBoardVo.board_idx}">
					<input type="hidden" name="member_idx" id="member_idx" value="${membership.member_idx}">
					<input type="hidden" name="board_idx" id="rd_down_board_idx" value="${reviewBoardVo.board_idx}">
					<input type="button" class="rd_down_button" name="rd_down" id="rd_down" value="비추천">
					</form>
					<input type="button" name="rd_down_count" id="rd_down_count" value="${reviewBoardVo.not_recommendation_count}" disabled>		
				</td>
			</tr>
			</c:when>
			<c:otherwise>
			...
			<tr>
				<td id="td_file" colspan="5">					
					첨부파일:
					<c:choose>
						<c:when test="${!empty reviewBoardVo.originFile}">
							<a href="download.do?oName=${reviewBoardVo.originFile}&sName=${reviewBoardVo.saveFile}">
							${reviewBoardVo.originFile}</a>
						</c:when>
						<c:otherwise>
						없음
						</c:otherwise>
				...
				<tr>
					<td class="reply_content" colspan="5">
						<form class="reply_edit_frm_class" name="reply_edit_frm">
							...
							<input type="hidden" name="board_idx" id="board_idx" value="${reviewBoardVo.board_idx}">
			...
			<tr>
				<td id="td_reply_paging" colspan="5">
					<%@ include file="../review_board/paging_reply.jsp" %>	
				</td>
			</tr>
			</table>
